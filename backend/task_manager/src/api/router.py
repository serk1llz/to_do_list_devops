...
async def count_completed(is_completed: bool = None, user: User = Depends(current_user)):
    try:
        async with async_session_maker() as session:
            result = await TaskService(user.id, session).get_count_by_completed(is_completed)
            if result['success']:
...
async def get_all_tasks(is_completed: bool = None, user: User = Depends(current_user),
    ...
    try:
        async with async_session_maker() as session:
            result = await TaskService(user.id, session).get_all_tasks(is_completed, paginator)
            if (result['success']) & (result['data'] is not None):
                output_tasks = [
...
async def add_task_for_user(params: TaskPOST, user: User = Depends(current_user)):
    try:
        async with async_session_maker() as session:
            result = await TaskService(user.id, session).add_task_for_user(params.title, params.target_date)
...
async def update_task_for_user(params: TaskPatch,
    ...
    try:
        async with async_session_maker() as session:
            result = await TaskService(user.id, session).update_task_for_user(**params.dict())
            if result['success']:
                return JSONResponse(status_code=status.HTTP_200_OK, content='Success')
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content='Not valid data')
    except Exception as e:
